...
deque<Digit> add(const deque<Digit>& a, const deque<Digit>& b)
{
    // Выясняем, какое число длиннее
    const deque<Digit>& long_num = a.size() >= b.size() ? a : b;
    const deque<Digit>& short_num = a.size() >= b.size() ? b : a;

    deque<Digit> ret; // Результат
    ...
    for (size_t index_short = short_num.size() - 1; index_short != size_t(-1); --index_short)
    {
        ...
        ret.push_front(sum % 10); // Добавляем цифру слева
    }
...
string to_string(const deque<Digit>& big_num)
{
    string ret;
...
void show(const deque<Digit>& a, const deque<Digit>& b)
{
