// Деление столбиком: оптимизация при делении на одну цифру
...
// Функция повторяет div_mod() из 4_long_div_1.cpp, только не вызывается div_chunk()
static pair<vector<Digit>, vector<Digit>> div_by_digit(const vector<Digit>& numerator, Digit denominator)
{
    ...
    DDigit chunk = 0; // Текущий кусок числителя (и одновременно остаток)
...
// Если знаменатель == 0, возвращает {0, 0}.
// Функция изменена по сравнению с 4_long_div_1.cpp
// (вызываем оптимизированную функцию, если в знаменателе одна цифра)
