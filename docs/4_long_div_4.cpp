// Деление столбиком: нормализация
...
Digit div_chunk(const vector<Digit>& chunk, const vector<Digit>& denominator)
{
    ...
    while (first_is_less(chunk, mul(vector<Digit>{digit}, denominator)))
    {
        --digit;
...
// Если знаменатель == 0, возвращает {0, 0}.
// Функция изменена по сравнению с 4_long_div_3.cpp (добавлена нормализация)
